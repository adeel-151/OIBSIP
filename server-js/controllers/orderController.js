...
export const createOrder = async (req, res, next) => {
    try {
        ...
        for (const item of items) {
            ...
            if (item.isCustom) {
                ...
                if (cleanItem.customIngredients) {
                    if (!mongoose.Types.ObjectId.isValid(cleanItem.customIngredients.base))
                        delete cleanItem.customIngredients.base;
                    if (!mongoose.Types.ObjectId.isValid(cleanItem.customIngredients.sauce))
                        delete cleanItem.customIngredients.sauce;
                    if (!mongoose.Types.ObjectId.isValid(cleanItem.customIngredients.cheese))
                        delete cleanItem.customIngredients.cheese;
        ...
        // DEDUCT INVENTORY
        for (const [id, reqQty] of Object.entries(requiredIngredients)) {
            await Inventory.findOneAndUpdate({ ingredientId: id }, { $inc: { quantity: -reqQty } }).catch(err => console.error('Error deducting inventory', err));
        }
        ...
        try {
            ...
            console.error('Socket error for inventory', err);
        }
        try {
...
export const verifyPayment = async (req, res, next) => {
    try {
        ...
        if (isValid) {
            const order = await Order.findOneAndUpdate({ razorpayOrderId: razorpay_order_id }, { paymentStatus: 'COMPLETED', razorpayPaymentId: razorpay_payment_id }, { new: true }).populate('user', 'name email').populate('items.pizza');
            if (order) {
                ...
                try {
                    ...
                    console.error('Socket/Email error', err);
                }
            }
